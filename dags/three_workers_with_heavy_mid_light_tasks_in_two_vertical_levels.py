...
@task(task_id="heavy_task_level_one", pool="pool_one", pool_slots=1)
def heavy_task_level_one_function():
    time.sleep(20)

@task(task_id="mid_task_level_one", pool="pool_two", pool_slots=1)   
def mid_task_level_one_function():
    time.sleep(12)

@task(task_id="light_task_level_one", pool="pool_three", pool_slots=1)
def light_task_level_one_function():
    time.sleep(3)
    
    
@task(task_id="heavy_task_level_two", pool="pool_one", pool_slots=1)
def heavy_task_level_two_function():
    time.sleep(20)

@task(task_id="mid_task_level_two", pool="pool_two", pool_slots=1)   
def mid_task_level_two_function():
    time.sleep(12)

@task(task_id="light_task_level_two", pool="pool_three", pool_slots=1)
...
def three_workers_with_heavy_mid_light_tasks_in_two_vertical_levels():
...
dag_definition = three_workers_with_heavy_mid_light_tasks_in_two_vertical_levels()
